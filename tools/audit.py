...
IGNORE = [
    ...
    # Контраст заголовков ленты меряется по полностраничному снимку, а у
    # .carousel свой горизонтальный скролл: WebKit рисует трек не там, где
    # getBoundingClientRect показал его мгновением раньше, и под надписью
    # оказывается пустое белое поле рядом с карточкой. Проверено снимком самой
    # карточки на 2880: белый заголовок лежит на тёмной воде, контраст ~10:1.
    # Гасим только ровно 1.0 — это чисто белая выборка, которой ни на одной
    # нашей фотографии быть не может; настоящий промах даёт дробное число и
    # сюда не попадёт. Попытка чинить замер (стоп rAF, обнуление scrollLeft,
    # снимок элемента) делала только хуже: промах перескакивал на другие
    # карточки, а поэлементный снимок ловил уже соседнюю.
    "(carousel__card-title, 19px): 1.0",
]
